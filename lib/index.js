...
export const Register = (components, actions = {}, models = {}) => {
  ...
  for(let name in components) {
    ...
    let memo = React.memo((props) => {
      let Element = React.createElement(components[name], props)
      ...
      if(Element._owner) {
        keep.owner = Element._owner
      }
      return <Element.type args={args} {...props} {...keep.models}/>
    })
    memo.name = name
    memo.displayName = name
    memo.mapReducer = components[name].map || {}
    memo.useOptions = components[name].options || {}
    /**
     * Bind actions to component
     */
